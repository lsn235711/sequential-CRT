...
#################################################################################
## Some parts of the code are adapted from the paper
## "Fast and Powerful Conditional Randomization Testing via Distillation"
## Please see https://github.com/moleibobliu/Distillation-CRT for more details
## Please contact the authors of that paper for data availability 
#################################################################################
...
# list of genes
gene_lst <- read.csv('data/ncomms11479-s4.csv', head = T)
...
cna_data <- read.csv('data/data_CNA.csv', head = T)
RNA_data <- fread('data/data_expression.txt')
...
clinical_data <- read.table('data/data_clinical_sample.csv', head = T,
                            sep = ',')
...
Sigma_est0 <- prec_est$Sigma

var_lst <- c()
for (indx in 1:p){
  beta_x <- solve(Sigma_est0[-indx, -indx], Sigma_est0[-indx, indx])
  X_bar <- X_pos[ ,-indx] %*% beta_x
  var_lst <- c(var_lst, mean((X_pos[,indx] - X_bar)^2))
  print(indx)
}
theta_lst <- diag(solve(Sigma_est0))
d <- sqrt(theta_lst * var_lst)
Sigma_est <- as.matrix(diag(d)) %*% as.matrix(Sigma_est0) %*% as.matrix(diag(d))

source("dCRT_functions.R")
...
# save(selected_sets, file = "selected_sets.rda")
# load(file = "selected_sets.rda")
# save.image(file = "work_space.RData")

set_d0 = RNA_data[1,d0CRT_results$select_set]
