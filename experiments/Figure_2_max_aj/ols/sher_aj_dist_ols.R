...
find_p_val = function(X, Y){
  ## for case where block size is 3 here
  cond_mean = cond_mean_12_22_inv[1] * (X[,neighbors_matrix[,1]] + X[,neighbors_matrix[,2]])
  cond_var = as.numeric(cond_sigma2)
  corr_0 = NULL
  for (j in 1:p){
    Xj_block = cbind(X[,j],X[,neighbors_matrix[j,1]], X[,neighbors_matrix[j,2]])
    corr_0 = c(corr_0, abs(lm(Y ~ Xj_block)$coefficients[2]))
  }
  corrs = NULL
  for (j in 1:p){
    Xj_block = cbind(X[,j],X[,neighbors_matrix[j,1]], X[,neighbors_matrix[j,2]])
    ...
    for (l in 1:p_value_precision){
      newXj = cond_mean[,j] + sqrt(cond_var)*rnorm(n)
      newX = Xj_block
      newX[,1] = newXj
      corrj = c(corrj, abs(lm(Y ~ newX)$coefficients[2]))
    }
    corrs = rbind(corrs, corrj)
  }
  p_val = (apply((corrs >= corr_0), 1, sum) + 1)/ (1 + p_value_precision)
  return(p_val)
}
...
print(max(ajs_mean))
save(seed,ajs_mean, file = paste("output_ols/output",seed,".Rdata", sep = ""))
