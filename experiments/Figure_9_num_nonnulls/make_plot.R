...
results = data.frame(fdp = numeric(0), power = numeric(0),
                     ...
                     one_shot = logical(0), k = integer(0),   
...
dat = dat0 %>% group_by(c, method, blackbox,include_h,knockoff_plus,setting_no, one_shot, k) %>%
...
p_FDR = ggplot(dat_toplot, aes(x=k, y=FDR,
...
p_power = ggplot(dat_toplot, aes(x=k, y= Power,
                                 group = interaction(`Variable Selection Method`, Blackbox),
                                 linetype = Blackbox,
                                 color = `Variable Selection Method`,
                                 shape = Blackbox
...
pp
pp %>% ggsave(file=sprintf("num_nonnull_experiment_%s.pdf", x_model), width=8.2, height=4.5, units="in")
